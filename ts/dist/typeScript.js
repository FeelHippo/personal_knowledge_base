...
const reverseString = (fullString) => {
    let reversedString = '';
...
const isPalindrome = (original) => original === reverseString(original);
console.log(isPalindrome('angelodidolegna'));
/// FizzBuzz
const fizzBuzzToN = (n) => new Array(n).fill(1).map((v, k) => helper(k + 1));
const helper = (k) => (!(k % 3) && !(k % 5) && console.log('FizzBuzz'), !(k % 3) && console.log('Fizz'), !(k % 5) && console.log('Buzz'));
fizzBuzzToN(100);
let data = [
    { id: 1, value: 4 },
    { id: 2, value: 3 },
...
console.log(new Set(data.map(({ value }) => !(value % 2) && // value is even
